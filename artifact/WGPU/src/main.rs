...
impl GPUBuffers {
    fn init(
        ...
        let spine: wgpu::Buffer = gpu.device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("Spine"),
            size: ((work_tiles * 4usize) * std::mem::size_of::<u32>()) as u64,
            usage: wgpu::BufferUsages::STORAGE,
            mapped_at_creation: false,
        });
...
impl Shaders {
    fn init(gpu: &GPUContext, gpu_buffers: &GPUBuffers) -> Self {
        ...
        let csdldf_stats_mod = gpu
            .device
            .create_shader_module(wgpu::include_wgsl!("../../SharedShaders/csdldf_stats.wgsl"));
        let csdldf_emu_mod = gpu
            .device
            .create_shader_module(wgpu::include_wgsl!("../../SharedShaders/csdldf_emulate.wgsl"));
        
        let memcpy_mod = gpu
            .device
            .create_shader_module(wgpu::include_wgsl!("../../SharedShaders/memcpy.wgsl"));
        ...
        let csdldf_stats = ComputeShader::init(gpu, gpu_buffers, "main", &csdldf_stats_mod, "CSDLDF Stats");
        let csdldf_emu = ComputeShader::init(gpu, gpu_buffers, "main", &csdldf_emu_mod, "CSDLDF Emulation");
...
impl PassLogic for RtsPass {
    fn main_pass(&self, tester: &Tester, com_encoder: &mut wgpu::CommandEncoder) {
        set_compute_pass(
            ...
            com_encoder,
            4096, //TODO change this to work tile size
            0u32,
        );
        set_compute_pass(
            &tester.gpu_context.query_set,
    ...
    fn validation_pass(&self, tester: &Tester) -> bool {
        return validate(tester, &tester.gpu_shaders.validate);
    }
...
impl PassLogic for CsdldfEmulatePass {
    fn main_pass(&self, tester: &Tester, com_encoder: &mut wgpu::CommandEncoder) {
        set_compute_pass(
            &tester.gpu_context.query_set,
...
impl PassLogic for MemcpyPass {
    ...
    fn validation_pass(&self, _tester: &Tester) -> bool {
        return true;
    }
...
impl Tester {
    ...
    async fn run(
        ...
        if lookback_length != 0u32 {
            ...
            let avg_lookback_length = lookback_length as f64 / batch_size as f64 / self.work_tiles as f64;
            println!("\nThread Blocks Launched: {}", self.work_tiles);
            println!("Average Total Spins per Pass: {}", avg_total_spins);
            println!("Average Lookback Length per Pass per Workgroup: {}", avg_lookback_length);
            println!(
    ...
    pub async fn run_test(
        ...
        let scan_type = match args[1].as_str() {
            ...
            "csdldf_emulate" => Some(ScanType::CsdldfEmulate),
        ...
        let pass: Box<dyn PassLogic> = match scan_type {
            ...
            ScanType::CsdldfEmulate => Box::new(CsdldfEmulatePass),
            ScanType::Memcpy => Box::new(MemcpyPass),
        };
...
pub async fn run_the_runner(args: Vec<String>) {
    ...
    let readback_size = 256u32;      //How many elements to readback, must be less than max
    tester
        .run_test(
            should_readback,
            should_time,
