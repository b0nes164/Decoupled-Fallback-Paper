...
\title{Decoupled-Fallback: A Portable Single-Pass Scan}
...
\begin{abstract}
  Scan is one of the most fundamental primitives in GPGPU computing, underpinning operations such as graph traversal and sorting. Although the state-of-the-art scan algorithm, \emph{Decoupled-Lookback}, achieves near-ideal (\emph{memcopy}) performance on NVIDIA hardware when implemented in CUDA, it relies on architectural and language-specific guarantees that no longer hold once outside of the NVIDIA ecosystem---most prominently, a forward progress guarantee (FPG). Without FPG, the algorithm becomes highly susceptible to deadlock, necessitating regression to slower approaches. We introduce \emph{Decoupled-Fallback}, a method that enables \emph{Decoupled-Lookback} to operate on hardware without FPG while avoiding deadlock, alongside other portability adaptations. On an FPG-lacking device---the Apple M1 Pro (8+2)---\emph{Decoupled-Fallback} achieves near-\emph{memcopy} speeds for inclusive prefix sum in WGPU and realizes the full theoretically expected 50\% speedup over the slower \emph{Reduce-Then-Scan} approach.
\end{abstract}
...
\begin{algorithm}
  ...
  \If{$\textnormal{highestRankLane}(\textit{threadid})$}{
    $\textit{wg\_partials}[\textit{subgroup\_id}] \gets \textit{local\_vectors}[\textnormal{final\_partial}]$\;
  }
  \textbf{barrier()}\;
...
\begin{algorithm}
  ...
  \KwSharedOut{ Post-scanned array $\textit{x}$ }
...
To reiterate, the goal of the lookback phase is twofold: to determine the reduction of all preceding work tiles and to communicate those values to succeeding tiles to accelerate their own lookback operations. Following the \emph{Decoupled-Lookback} technique, we model the inter-workgroup spine as a finite state machine, where each work tile comprising the spine may exist in one of three states:
\begin{itemize}
...
\end{itemize}
As the spine resides in global memory, all tiles are initialized to the \emph{Not Ready} state prior to kernel launch, with their corresponding reduction values set to the identity element of the monoid. Over the course of the lookback, each tile progresses from \emph{Not Ready} to \emph{Ready}, then to \emph{Inclusive}, and once advanced, may never revert to a prior state. Although workgroups depend on preceding tiles, they operate independently and are primarily responsible for updating their own state, only opportunistically updating other tiles' states in the event of a fallback. To illustrate this process, we present the flowchart in Figure~\ref{fig:decoupled_fallback}.
