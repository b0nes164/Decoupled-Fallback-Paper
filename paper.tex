...
\emph{Chained Scans} are \emph{one-way producer-consumer} algorithms; each workgroup consumes reductions only from \emph{lower-rank}\footnote{We specifically use the term \emph{rank} here to generalize between which ID---atomically acquired or scheduler-assigned---is used to determine this ordering. This distinction matters because the choice of ID determines which progress model, as defined by Sorensen et al.~\cite{sorensen2021}, is required to guarantee termination. Working under the assumption that the scheduler-assigned ID is used, Sorensen et al.~\cite{sorensen2021, sorensen2018} classify \emph{Chained Scan} as requiring \emph{HSA} (Heterogeneous System Architecture), a model that guarantees progress for the lowest-ranked workgroup. However, if the atomically acquired ID determines rank, then the HSA guarantee no longer suffices, as the lowest scheduler-assigned ID may not correspond to the lowest atomic ID. In this case, termination requires only \emph{OBE} (Occupancy-Bound Execution), which guarantees progress once a workgroup begins execution.} counterparts, thereby enforcing the ordering required to elide barriers. For such algorithms, guaranteeing execution of the lowest-rank workgroup is sufficient to ensure termination. If subgroups from the lowest-rank workgroup are not guaranteed execution, dependent workgroups may spin indefinitely, waiting for updates that never arrive. A \emph{forward-progress guarantee}, in the sense required by \emph{Chained Scan}, ensures that all occupant subgroups---potentially from different workgroups---are eventually granted execution after becoming active, thus preventing indefinite starvation.
...
\begin{figure*}
  ...
  \caption{\emph{Decoupled Fallback}. For brevity, the lookback is condensed; see Figure~\ref{fig:decoupled-lookback} for the full behavior, which is shared with \emph{Decoupled Lookback}.}
  ...
  \ForEach{$\textit{thread\_id}$ \textbf{in} $\textit{W}$ \textbf{in parallel}}{
    \For{$\textit{j} \gets \textit{S}$ \KwTo $\textit{alignment}$ \textbf{with} $\textit{j} \gets \textit{j} \cdot \textit{S}$}{
      ...
      \If{$\textit{thread\_id} < \textit{step}$}{
        $\textit{temp} \gets \text{subgroupKoggeStone}(\textit{x}[\textit{thread\_id} + \textit{top\_offset}])$\;
      ...
      \If{$\textit{j} \neq \textit{S}$}{
        ...
        $\textit{cond2} \gets \textit{fanout\_index} \bmod \textit{j} \geq \textit{reduced\_stride}$\;
