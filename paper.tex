...
The absence of FPG in key hardware segments poses a critical challenge for developers. Our testing ~\ref{fig:dist} shows that, at best, running \emph{Chained Scan} without FPG results in performance worse than \emph{Reduce-then-Scan}, and frequently triggers \emph{Timeout Device Recovery}\footnote{Operating systems handle unresponsive programs differently. On Windows, the Timeout Detection and Recovery (TDR) system resets the GPU after approximately 2 seconds. macOS has a timeout watchdog that, interestingly, does not terminate a spinning kernel---allowing us to obtain these results. On Linux, behavior varies by GPU driver.} (TDR), crashing the program.
\begin{figure}
  \centering
  \includegraphics[width=0.6\linewidth]{graphics/dist.pdf}
  \caption{Execution times of \emph{Decoupled Lookback} on an FPG-lacking device, Apple M1 Max. The TDR threshold represents the duration after which the OS, assuming a non-terminating kernel, kills the driver.\label{fig:dist}}
\end{figure}
