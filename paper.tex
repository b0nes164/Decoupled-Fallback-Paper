...
\acmConference[SPAA '25]{37th ACM Symposium on Parallelism in Algorithms and Architectures}{July 28--August 1, 2025}{Portland, OR, USA}
\acmBooktitle{37th ACM Symposium on Parallelism in Algorithms and Architectures (SPAA '25), July 28--August 1, 2025, Portland, OR, USA}
\acmDOI{10.1145/3694906.3743326}
\acmISBN{979-8-4007-1258-6/25/07}
...
\begin{figure*}[!t]
  ...
  \makebox[\textwidth][c]{
    \begin{subfigure}[c]{0.48\textwidth}
      \centering
      \includegraphics[height=7.5cm,keepaspectratio]{graphics/ScanThenPropagate.pdf}
      \caption{\emph{Scan-then-Propagate.}}
      \label{fig:scan-then-propagate}
    \end{subfigure}
    \hfill
    \begin{subfigure}[c]{0.48\textwidth}
      \centering
      \includegraphics[height=6cm,keepaspectratio]{graphics/ReduceThenScan.pdf}
      \caption{\emph{Raking Reduce-then-Scan.}}
      \label{fig:reduce-then-scan}
    \end{subfigure}%
  }\\[1em]
  ...
  \makebox[\textwidth][c]{
    \begin{subfigure}[c]{0.48\textwidth}
      \centering
      \includegraphics[height=6cm,keepaspectratio]{graphics/StreamScan.pdf}
      \caption{\emph{Stream Scan.}}
      \label{fig:stream-scan}
    \end{subfigure}
    \hfill
    \begin{subfigure}[c]{0.48\textwidth}
      \centering
      \includegraphics[height=6cm,keepaspectratio]{graphics/DecoupledLookback.pdf}
      \caption{\emph{Decoupled Lookback.}}
      \label{fig:decoupled-lookback}
    \end{subfigure}%
  }

  \caption{The four global scan techniques described in Section~\ref{sec:Evolution}. Dashed grey boxes indicate individual kernel launches.}
  \label{fig:four-techniques}
\end{figure*}
...
Moreover, the fused nature of \emph{Chained Scan} enables in-situ compaction: algorithms requiring compaction can merge that work directly into the scan kernel, avoiding redundant writes or secondary compaction passes. This fusion benefits applications ranging from data deduplication to compression. However, \emph{Chained Scan} has a critical limitation: its reliance on a forward-progress guarantee. While NVIDIA GPUs provide this guarantee, data collected by Sorensen et al.~\cite{sorensen2021} shows that ARM and Apple GPUs do not. Without forward-progress guarantees, \emph{Chained Scan} is not merely inefficient---it risks indefinite starvation, causing the application to freeze and rendering the kernel unable to terminate, requiring external intervention to recover.

\subsection{Why is Reliance on Forward-Progress Guarantees a Portability Problem?}
\emph{Chained Scans} are \emph{one-way producer-consumer} algorithms; each workgroup consumes reductions only from lower-ID counterparts, thereby enforcing the ordering required to elide barriers. For such algorithms, guaranteeing execution of the lowest-ID workgroup is sufficient to ensure termination. If subgroups from the lowest-ID workgroup are not guaranteed execution, dependent workgroups may spin indefinitely, waiting for updates that never arrive. A \emph{forward-progress guarantee} ensures that all occupant subgroups---potentially from different workgroups---are eventually granted execution, thus preventing indefinite starvation.

The absence of FPG in key hardware segments poses a critical challenge for developers. Our testing (Figure~\ref{fig:dist}) shows that, at best, running \emph{Chained Scan} without FPG results in performance worse than \emph{Reduce-then-Scan}, and frequently triggers \emph{Timeout Device Recovery}\label{sec:tdr}\footnote{Operating systems handle unresponsive programs differently. On Windows, the Timeout Detection and Recovery (TDR) system resets the GPU after approximately 2 seconds. Other operating systems—such as macOS and Linux—employ their own timeout mechanisms with varying behaviors. For consistency, we adopt Windows’s 2-second limit as a reasonable expectation for maximum execution time.} (TDR), crashing the program. Exacerbating this issue, no major graphics API provides a mechanism to query FPG support. This lack of transparency forces developers to design algorithms without knowing whether critical guarantees are available on the target hardware. In shader environments, where cross-platform portability is essential, this uncertainty compels a fallback to older, less efficient strategies like \emph{Reduce-then-Scan}, sacrificing performance for broader compatibility.
...
\begin{figure*}[t]
  ...
  \caption{\emph{Decoupled Fallback}. For brevity, the chained propagation phase is condensed; see Figure~\ref{fig:decoupled-lookback} for the full behavior.}
  ...
  \ForEach{$\textit{thread\_id}$ \textbf{in} $\textit{W}$ \textbf{in parallel}}{
    \For{$\textit{j} \gets \textit{S}$ \KwTo $\textit{alignment}$ \textbf{with} $\textit{j} \gets \textit{j} \cdot \textit{S}$}{
      ...
      \If{$\textit{thread\_id} < \textit{step}$}{
        $\textit{temp} \gets \text{subgroupKoggeStoneScan}(\textit{x}[\textit{thread\_id} + \textit{top\_offset}])$\;
...
\begin{table*}
  ...
  \begin{tabular*}{\textwidth}{@{\extracolsep{\fill}} l r r r r r r}
    ...
    \midrule
    Avg \emph{Memcpy} (32-bit ele/sec $\cdot 10^9$)      & 1.48    & 2.50     & 10.82   & 37.42    & 50.95    & 73.68    \\
    Avg \emph{DF} (32-bit ele/sec $\cdot 10^9$)          & 1.51    & 2.65     & 10.87   & 36.85    & 51.25    & 83.27    \\
    Avg \emph{RTS} (32-bit ele/sec $\cdot 10^9$)         & 1.06    & 1.94     & 7.46    & 25.75    & 34.36    & 62.57    \\
    \emph{DF}/\emph{Memcpy}                              & 102.0\% & 106.0\%  & 100.5\% & 98.4\%   & 100.6\%  & 113.0\%  \\
    \emph{DF}/\emph{RTS}                                 & 142.6\% & 136.6\%  & 145.7\% & 143.1\%  & 149.1\%  & 133.1\%  \\
    Avg Spins per workgroup                              & 0.420   & 2.52     & 0.719   & 0.752    & 0.679    & 0.533    \\
    Avg Lookback Length per workgroup                    & 1.714   & 3.52     & 4.598   & 10.737   & 12.350   & 22.648   \\
    Avg Total Fallbacks Initiated                        & 446.183 & 4739.91  & 449.961 & 1046.216 & 333.324  & 266.641  \\
    Avg Total Successful Insertions                      & 21.475  & 571.24   & 23.887  & 4.822    & 11.378   & 46.288   \\
    \bottomrule
  \end{tabular*}
...
As shown in Table~\ref{tab:results}, \emph{DF} achieves at least a $\sim$40\% speedup over \emph{RTS} across all tested devices, closely approaching the theoretically expected $O(3n)/O(2n)$ 50\% speedup, except on the 7900XT and Mali-G78. This result initially appears confusing, given that \emph{DF} outperforms \emph{Memcpy} by 6\% and 13\%, respectively. However, we hypothesized and confirmed a secondary effect of \emph{Chained Scans}: they enforce a queue-like behavior among workgroups. In-flight workgroups with a lower relative rank traverse fewer tiles during their lookbacks, enabling them to exit earlier than higher-ranked workgroups. Because this ordering emerges naturally from differing amounts of computation rather than explicit synchronization, the exit sequence is \emph{k-sorted} or \emph{nearly ordered}, rather than strictly ordered. Scheduling fairness and message-passing visibility can further influence this sequence---for example, an unfair scheduler might allocate more resources to a higher-ranked workgroup, causing it to exit before a lower-ranked counterpart, or a higher-ranked workgroup might observe an updated flag sooner, enabling earlier completion. We confirmed this behavior experimentally by using atomics to record and print the order in which workgroups completed their lookback phase. Unlike a simple \emph{Memcpy} kernel, where all in-flight workgroups begin writing immediately after completing reads, this “ordered exit” pattern facilitates more efficient global memory accesses during the write phase of the scan, as adjacent workgroups are more likely to share memory pages, reducing premature page evictions. We hypothesize that the 7900XT and Mali-G78 have insufficient memory systems to efficiently accommodate all in-flight workgroups, making them particularly sensitive to this effect. To test this hypothesis, we augmented our \emph{Memcpy} kernel with a simplified lookback routine---excluding the scan itself---and observed a performance improvement of approximately 10\% on the 7900XT\@. Thus, the unexpectedly slow baseline \emph{Memcpy} performance on these devices explains why \emph{DF}, despite its strong overall performance, does not fully achieve the theoretical 50\% speedup over \emph{RTS}.
...
\begin{acks}
We thank Elias Naur for his initial work on Scalar Fallback, Ken Russell for facilitating this collaboration, Duane Merrill for answering our questions about scan terminology, and Oliver Le for providing the initial testing on Apple devices.
\end{acks}
