...
\begin{algorithm}[htb]
  ...
  \KwIn{%
    ...
    \texttt{wg\_lock},
    \texttt{wg\_partials}[],
    \texttt{wg\_prev\_reduction},
    \texttt{wg\_broadcast},
  }
  \KwOut{\texttt{out}[] fully inclusive scan of \texttt{in}[]}

  \If{$\textnormal{threadid.x} = 0$}{
    \texttt{wg\_broadcast} $\gets$ \texttt{atomicAdd}(\&\texttt{tileBump}, 1)\;
    \texttt{wg\_lock} $\gets$ \textnormal{LOCKED}\;
  }
  \textbf{barrier()}\;

  \texttt{part\_id} $\gets$ \texttt{wg\_broadcast}\;

  \textit{local\_vectors}: $\textnormal{array}\langle \textnormal{vec4}\langle u32\rangle, p\rangle$\;

  \textbf{Load}(\texttt{in}, \texttt{part\_id}, \textit{local\_vectors})\;
  \textbf{SubgroupRake}(\textit{local\_vectors})\;

  \If{$\text{\normalfont highestRankLane}(\textnormal{threadid.x})$}{
    \texttt{wg\_partials}[\textnormal{subgroup\_id}] $\gets$ \textit{local\_vectors}[\textnormal{final\_partial}]\;
  }
  \textbf{barrier()}\;

  \textbf{WorkgroupWideScan}(\texttt{wg\_partials})\;
  \textbf{barrier()}\;

  \tcp{Decoupled Fallback}
  \While{\texttt{wg\_lock} = \textnormal{LOCKED}}{
    \textbf{Lookback}(\texttt{spine}, \texttt{wg\_prev\_reduction}, \texttt{wg\_lock})\;
    \texttt{lock} $\gets$ \textbf{\texttt{workgroupUniformLoad}}(\&\texttt{wg\_lock})\;

    \If{$\texttt{lock} = \textnormal{LOCKED}$}{
      \textbf{FallbackReduce}(\texttt{in}, \texttt{wg\_fallback})\;
      \textbf{FallbackInsertionAttempt}(\texttt{spine}, \texttt{wg\_prev\_reduction}, \texttt{wg\_lock})\;
      \texttt{lock} $\gets$ \textbf{\texttt{workgroupUniformLoad}}(\&\texttt{wg\_lock})\;
    }
  }

  \textbf{Write}(\texttt{out}, \textit{local\_vectors}, \texttt{wg\_partials}, \texttt{wg\_prev\_reduction}, \texttt{part\_id})\;

  \caption{High-level Decoupled Fallback}
\end{algorithm}
