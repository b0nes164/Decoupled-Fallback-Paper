...
To reiterate, the goal of the lookback is twofold: to determine the reduction of all preceding work tiles and to relay those values to succeeding tiles to accelerate their own lookback operations. Following the \emph{Decoupled-Lookback} technique, we model the inter-workgroup spine as a finite-state machine, where each work tile comprises a reduction value and a state, stored in global memory. Each tile may exist in one of three states:
\begin{itemize}
...
\end{itemize}
Each workgroup is responsible for updating its own state, only opportunistically updating other tiles' states in the event of a fallback. A tile can only transition forward---Not Ready \rightarrow Ready \rightarrow Inclusive—and once advanced, may never revert to a prior state. Before kernel launch, all tiles in the spine are initialized to \emph{Not Ready}, and their reduction values are set to $\epsilon$. Workgroups operate independently but remain dependent on preceding tiles to finalize their own inclusive reductions. As they transition through states, workgroups progressively accumulate reductions from prior tiles and, once their lookback phase is complete, post their inclusive reduction for the next tile to consume.
...
To coordinate this process, each workgroup initializes a shared memory \emph{control flag} during partition index acquisition. This flag governs both the initiation of fallback operations and the entry and exit of the entire \emph{Decoupled-Fallback} phase, which begins immediately after the workgroup-wide scan over partial reductions is completed. The details of tile state encoding and atomic updates are discussed in the following section and may vary depending on the data type, so for simplicity, we describe the lookback here as being performed by a single thread.

\john{Also, we present the algorithm from the perspective of one workgroup, which at all times has a pointer to a ``current tile'' on which it is working. (Sloppily worded, sorry.)}
...
\thomas{I'm going to use ``ingest'' to match the flow chart. I literally used a thesaurus to choose accumulate . . . Can still change later.}

We now detail the \emph{Decoupled-Fallback} procedure, presenting the algorithm from the perspective of a single workgroup:
\begin{figure}
  \centering
...
\begin{enumerate}
  \item[(0)] \textbf{Posting Local Reduction:} 
        \begin{itemize}
          ...
          \item If it is the first work tile, its local reduction \emph{is} its inclusive reduction. In this case, the workgroup updates its tile state directly to \emph{Inclusive} and bypasses the lookback entirely.
        \end{itemize}

  \item \textbf{Initialization:}
        \begin{itemize}
          \item The control flag, initialized to \emph{Locked}, signals that the workgroup should enter the lookback phase.
          \item The lookback thread initializes a \emph{previous reduction} variable in registers with $\epsilon$ to hold the accumulated reductions of preceding tiles.
          \item To traverse the spine, the lookback thread initializes a \emph{current tile} pointer to its immediate predecessor.  
        \end{itemize}

  \item \textbf{Lookback:}
        \begin{itemize}
          \item The lookback thread queries the state of the \emph{current tile}.
          ...
          \item If the tile is in the \emph{Not Ready} state, the lookback thread spins for a limited number of iterations, defined by the \emph{Maximum Spin Count}. If this count is exceeded, the thread exits the lookback loop, leaving the control flag \emph{Locked}, signaling to the workgroup that a fallback is required. To prepare for the fallback, it stores a copy of the \emph{current tile} in shared memory for the workgroup to process. Without FPG, \emph{Decoupled-Lookback} risks spinning indefinitely and deadlocking at this point.
          
          \john{I don't get ``signals''. To whom are we signaling? What is signaled?}
          \john{I think this is the place where decoupled-lookback fails without FPG\@. If so, say so, and why it fails. It underscores the central message of the paper, and is thus worth reiterating.}
        \end{itemize}
  ...
  \item \textbf{Finalization:}
        \begin{itemize}
          ...
          \john{Be specific about what happens here! I'm presuming part of this item's work + the previous item's work is ``use the previous reduction as the starting point for scanning then outputting our current tile''. ``subsequent operations'' is super non-specific.} 
