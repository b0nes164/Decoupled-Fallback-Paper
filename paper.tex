...
What is the exact problem that is being solved?
Why is this problem important?
Why is this problem hard / why aren't the current solutions to this problem sufficient? (In our case, that's going to be an assertion that current techniques don't work without fpg, but we will certainly not explain it in detail at this point; that's going to be a whole section later in the paper.)
What are the precise contributions of the work?

Scan is one of the most ubiquitous problems in GPGPU computing, with applications including sorting, sparse matrix etc.  Although the state of the art prefix scan, Merrill and Garland's \emph{Decoupled Lookback}, achieves near-ideal performance on NVIDIA hardware and implemented in CUDA, it relies on a set of architectural and language-specific guarantees that no longer hold once outside of the NVIDIA ecosystem, most notably a property of a device's scheduler known as a \emph{forward progress guarantee}, without which \emph{Decoupled Lookback} is liable to starve indefinitely.

\emph{Decoupled Lookback} belongs to a class of scan architectures known as \emph{Chained Scan}, which hybridizes serial and parallel strategies to read and write each element exactly once, achieving the theoretical minimum global memory movement of $O(2n)$. When this algorithm fully saturates the device’s memory bandwidth, it reaches the \emph{Speed of Light} (SOL) performance limit, the theoretical peak throughput constrained by available bandwidth and memory latency. As the compute capabilities of major GPUs have grown, there has been increasing interest in bringing Chained Scan techniques into broader use. However, the absence of a critical scheduling guarantee has forced developers to revert to slower techniques to maintain compatibility across architectures.
...
We present \textit{Chained Scan with Decoupled Lookback and Decoupled Fallback}, a fully portable prefix scan capable of reaching speed of light performance without FPG, suitable for the shading language environment (graphics APIs?), and implemented in WGSL. (REWORD THIS) Additionally, we introduce a tile state representation for \emph{Chained-Scans} that does not rely on 64-bit atomics or memory barriers for correctness, along with a subgroup-size-agnostic intra-workgroup implementation.
...
The viability of \emph{Decoupled-Fallback} relies on the same cache locality properties as \emph{Decoupled-Lookback}. Since no subgroup scheduler is perfectly fair, all hardware inherently has a natural fallback rate $f$\!, independent of FPG, due to false-positive fallback operations. As a result, memory bandwidth usage increases (and we expect throughput to decrease) by $O(fn)$ to $O((2 + f)n)$. However, just as serial ordering confines spine memory accesses within an $o$-width sliding window, scan input accesses are likewise constrained to an $ot$-width window, increasing their likelihood of residing in cache. As we later empirically demonstrate in \ref{sec:Sim-Blocking}, this cache residency significantly reduces memory access costs, making fallback operations predominantly compute-bound when reducing a blocking tile. Since pure reduction has minimal overhead, throughput approaches $O(2n)$ on most devices.
...
\begin{table*}
  ...
  \begin{tabular*}{\textwidth}{@{\extracolsep{\fill}} l r r r r r r}
    ...
    \bottomrule
  \end{tabular*}
  \caption{\emph{Decoupled Fallback} performance, 32-bit Inclusive Prefix Sum, $\mathbf{2^{25}}$ 32-bit elements}
  \label{tab:results}
\end{table*}
...
To better understand how devices interact with \emph{DF}, we also collect a number of statistics during its execution:
\begin{itemize}
