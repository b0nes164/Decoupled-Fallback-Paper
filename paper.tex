...
To optimize the fallback operation, we employ three strategies. Although \emph{Chained-Propagation} is performed by a single thread or subgroup, we want the entire workgroup to participate in the fallback. First, a shared memory \emph{control flag} enables threads performing the lookback to signal the entire workgroup when a fallback is necessary. Second, we limit the number of spins a thread can perform while waiting on a tile; exceeding this threshold triggers a fallback. While this approach risks redundant fallbacks---since a late tile is indistinguishable from a genuinely blocking one---exposing the maximum spin count as a tunable parameter helps minimize false positives. Finally, after completing a fallback, the workgroup attempts to insert the reduction in place of the blocking tile, preventing redundant fallbacks by subsequent workgroups.
...
\begin{enumerate}
  ...
  allowing the subgroup-block scan to remain barrier-free and in registers.The subgroup-block reduction is then posted to shared memory. For a workgroup of size $w$, subgroup-raking reduces the local spine to $\frac{w}{s}$.
  ...
  \item \textbf{Chained Propagation, Decoupled Fallback}: The first subgroup in the workgroup performs a lookback operation, engaging in fallback operations if necessary. Once completed, it posts the results to shared memory for the workgroup to consume.
...
\end{itemize}
Workgroups operate independently, each responsible for updating its own tile state but may opportunistically update preceding tiles in the event of a fallback. Tile states transition strictly forward---Not Ready $\rightarrow$ Ready $\rightarrow$ Inclusive---and never revert. Before kernel launch, all tiles are initialized to \emph{Not Ready} with reduction values set to $\epsilon$. State transitions dictate reduction accumulation: at \emph{Ready}, local reductions are posted; at \emph{Inclusive}, preceding values are incorporated. To coordinate this process, each workgroup initializes a shared memory \emph{control flag} during partition index acquisition. This flag governs both the initiation of fallback operations and the entry and exit of the entire \emph{Decoupled-Fallback} phase, which begins immediately after the workgroup-wide scan over partial reductions is completed.
...
\begin{enumerate}
  ...
  \item \textbf{Initialization:}
        \begin{itemize}
          ...
          \item To traverse the spine, the first thread initializes a \emph{current tile} pointer to its immediate predecessor.
        \end{itemize}
          ...
          \item The first thread queries the state of \emph{current tile}.
                \begin{itemize}
                  ...
                  \item If the tile is in the \emph{Not Ready} state, the first thread spins for a limited number of iterations, defined by the \emph{Maximum Spin Count}.
                        \begin{itemize}
                            \item If this count is exceeded, it exits the lookback loop, leaving the control flag \emph{Locked} to signal to the workgroup that a fallback is required. Additionally, it copies \emph{current tile} into shared memory to broadcast the fallback location to the workgroup.
                            \item Else, it continues spinning until the tile transitions to a \emph{Ready} or \emph{Inclusive} state. Without FPG, \emph{Decoupled Lookback} risks spinning indefinitely and deadlocking at this stage.
                        \end{itemize}
                \end{itemize}
        \end{itemize}
  ...
  \item \textbf{Fallback:}
        \begin{itemize}
          ...
          \item As this update attempt is performed atomically, the workgroup reads back the blocking tile's most recent state:
                \begin{itemize}
                  \item If the state is \emph{Inclusive}, the fallback value computed by the workgroup is discarded, and the first thread ingests the returned value before proceeding to step 4.
                  \item Else, the first thread ingests the fallback value.
                        \begin{itemize}
                            \item If the blocking tile is the first in the spine, the workgroup has reached the start of the spine and thus completed the lookback. The first thread proceeds to step 4.
                            \item Else, the first thread updates \emph{current tile} to the preceding tile, continuing the lookback process at step 2.
                        \end{itemize}
                \end{itemize}
        \end{itemize}
  ...
  \item \textbf{Finalization:}
        \begin{itemize}
          \item To prioritize reduction propagation, the first thread immediately adds the local reduction to \emph{previous reduction}, posts the inclusive reduction to global memory, and transitions the tile state to \emph{Inclusive}.
...
Fallbacks require a tile state representation and an atomic operation that can safely transition a tile to \emph{Ready} without overwriting a more advanced state. When an update attempt is made, a blocking tile may be in any of the three states due to two scenarios: a false-positive fallback on a tile that transitions to \emph{Ready} or \emph{Inclusive} before fallback completion; or another workgroup completes its fallback first, leaving the tile in \emph{Ready}. WebGPU does not guarantee the correctness of atomic \emph{Compare-and-Swap} across all devices, prohibiting its use. Instead, we encode the tile state in the most significant bits of the split value and enforce a strict state ordering: \emph{Inclusive} $>$ \emph{Ready} $>$ \emph{Not Ready}. This allows atomic \emph{Maximum}, which inherently preserves state hierarchy, to safely perform updates.
